import {CYPER_EVENTS, EVENTS, EVENTS_ERROR} from "../../constant";
import screenfull from "screenfull";
import {getTarget, isMobile} from "../../utils";
...
export default (player) => {
    ...
    try {
        const screenfullChange = (e) => {
            if (getTarget(e) === player.$container) {
                player.emit(CYPER_EVENTS.fullscreen, player.fullscreen)
    ...
    if (isMobile()) {
        player.on(EVENTS.webFullscreen, (value) => {
            if (value) {
                player.$container.classList.add('cyperstream-fullscreen-web')
            } else {
                player.$container.classList.remove('cyperstream-fullscreen-web')
            }
            //
            player.emit(CYPER_EVENTS.fullscreen, player.fullscreen)
        })
